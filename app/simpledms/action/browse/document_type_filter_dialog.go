...
func (qq *DocumentTypeFilterDialog) Widget(ctx ctxx.Context, data *DocumentTypeFilterDialogData, listDirState *ListDirState) *wx.Dialog {
	return &wx.Dialog{
		...
		Child: qq.actions.DocumentTypeFilter.Widget(
			ctx,
			qq.actions.DocumentTypeFilter.Data(data.CurrentDirID),
			listDirState,
		),
	}
}
