...
func (qq *TagContextMenu) Widget(ctx ctxx.Context, fileID string, tagx *enttenant.Tag) *wx.Menu {
	deleteLink := &wx.MenuItem{
		LeadingIcon: "delete",
		Label:       wx.T("Delete"),
		HTMXAttrs: wx.HTMXAttrs{
			HxPost: qq.actions.DeleteTag.Endpoint(),
			HxVals: util.JSON(qq.actions.DeleteTag.Data(tagx.ID)),
	...
	menuItems := []*wx.MenuItem{
		{
			LeadingIcon: "edit",
			Label:       wx.T("Edit"),
			HTMXAttrs: qq.actions.EditTag.ModalLinkAttrs(
				qq.actions.EditTag.Data(tagx.ID, tagx.Name),
		...
		if groupCount > 0 {
			menuItems = append(
				...
				&wx.MenuItem{
					...
					HTMXAttrs: qq.actions.MoveTagToGroup.ModalLinkAttrs(
						qq.actions.MoveTagToGroup.Data(tagx.ID, 0),
