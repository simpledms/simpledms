...
func (qq *ListDirPartial) fileTable(
	ctx ctxx.Context,
	...
	rows := make([]*widget.TableRow, 0, len(queryResult.Children))
	for _, child := range queryResult.Children {
		rows = append(rows, qq.fileTableRow(
			ctx,
			state,
			data,
			child,
			queryResult.ChildParentFullPaths,
			preferences,
			spaceColumns,
			propertyColumns,
			tagGroupColumns,
	...
	if queryResult.HasMore {
		rows = append(rows, qq.fileTableLoadMoreRow(offset, len(columns)))
	}
...
func (qq *ListDirPartial) fileTableLoadMoreRow(offset int, columnCount int) *widget.TableRow {
	...
	return &widget.TableRow{
		Widget: widget.Widget[widget.TableRow]{
			ID: "listDirLoadMoreTable",
		},
		Cells: cells,
		HTMXAttrs: widget.HTMXAttrs{
			HxPost:    qq.Endpoint() + "?offset=" + fmt.Sprintf("%d", offset+qq.pageSize()),
			HxTrigger: "intersect once",
			HxTarget:  "#listDirLoadMoreTable",
			HxSwap:    "outerHTML",
		},
	}
}

func (qq *ListDirPartial) fileTableColumns(
	preferences *filelistpreference.FileListPreferences,
...
func (qq *ListDirPartial) fileTableRow(
	ctx ctxx.Context,
	state *ListDirPartialState,
	data *ListDirPartialData,
	...
	if preferences.HasBuiltInColumn(filelistpreference.FileListColumnName) {
		cells = append(cells, &widget.TableCell{Child: qq.fileTableNameCell(ctx, state, data, filex, childParentFullPaths)})
		hasData = true
	}
	if preferences.HasBuiltInColumn(filelistpreference.FileListColumnOriginalFilename) {
		addCell(filex.Name)
...
func (qq *ListDirPartial) fileTableNameCell(
	ctx ctxx.Context,
	state *ListDirPartialState,
	data *ListDirPartialData,
	filex *enttenant.File,
	...
	namePrefix := ""
	withDocumentType := false
	if state.IsRecursive && ctx.SpaceCtx().Space.IsFolderMode {
		parentFullPath := childParentFullPaths[filex.ParentID]
		currentDirPath := qq.infra.FileSystem().FileTree().FullPathByPublicIDX(ctx, data.CurrentDirID)
		parentFullPath = strings.TrimPrefix(parentFullPath, currentDirPath+string(os.PathSeparator))
		if parentFullPath != "" {
			namePrefix = parentFullPath + " » "
...
func (qq *ListDirPartial) fileTableRowHTMXAttrs(
	ctx ctxx.Context,
	currentDirID string,
	filex *enttenant.File,
) widget.HTMXAttrs {
...
func (qq *ListDirPartial) fileTableColumnData(
	ctx ctxx.Context,
...
func (qq *ListDirPartial) fileTablePropertyColumns(
	ctx ctxx.Context,
	...
	properties := ctx.SpaceCtx().TTx.Property.Query().
...
func (qq *ListDirPartial) fileTableTagGroupColumns(
	ctx ctxx.Context,
