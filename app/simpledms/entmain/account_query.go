...
func (aq *AccountQuery) QueryTenants() *TenantQuery {
...
// QueryReceivedMails chains the current query on the "received_mails" edge.
func (aq *AccountQuery) QueryReceivedMails() *MailQuery {
	query := (&MailClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryTemporaryFiles chains the current query on the "temporary_files" edge.
func (aq *AccountQuery) QueryTemporaryFiles() *TemporaryFileQuery {
	query := (&TemporaryFileClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryTenantAssignment chains the current query on the "tenant_assignment" edge.
func (aq *AccountQuery) QueryTenantAssignment() *TenantAccountAssignmentQuery {
	query := (&TenantAccountAssignmentClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withTenants:          aq.withTenants.Clone(),
		withReceivedMails:    aq.withReceivedMails.Clone(),
		withTemporaryFiles:   aq.withTemporaryFiles.Clone(),
		withTenantAssignment: aq.withTenantAssignment.Clone(),
...
func (aq *AccountQuery) WithTenants(opts ...func(*TenantQuery)) *AccountQuery {
	query := (&TenantClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withTenants = query
	return aq
}

// WithReceivedMails tells the query-builder to eager-load the nodes that are connected to
// the "received_mails" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithReceivedMails(opts ...func(*MailQuery)) *AccountQuery {
	query := (&MailClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withReceivedMails = query
	return aq
}

// WithTemporaryFiles tells the query-builder to eager-load the nodes that are connected to
// the "temporary_files" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithTemporaryFiles(opts ...func(*TemporaryFileQuery)) *AccountQuery {
	query := (&TemporaryFileClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withTemporaryFiles = query
	return aq
}

// WithTenantAssignment tells the query-builder to eager-load the nodes that are connected to
// the "tenant_assignment" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithTenantAssignment(opts ...func(*TenantAccountAssignmentQuery)) *AccountQuery {
	query := (&TenantAccountAssignmentClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withTenantAssignment = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	var (
		...
		loadedTypes = [4]bool{
			aq.withTenants != nil,
			aq.withReceivedMails != nil,
			aq.withTemporaryFiles != nil,
			aq.withTenantAssignment != nil,
		}
	)
	...
	if query := aq.withTenants; query != nil {
		if err := aq.loadTenants(ctx, query, nodes,
			func(n *Account) { n.Edges.Tenants = []*Tenant{} },
			func(n *Account, e *Tenant) { n.Edges.Tenants = append(n.Edges.Tenants, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withReceivedMails; query != nil {
		if err := aq.loadReceivedMails(ctx, query, nodes,
			func(n *Account) { n.Edges.ReceivedMails = []*Mail{} },
			func(n *Account, e *Mail) { n.Edges.ReceivedMails = append(n.Edges.ReceivedMails, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withTemporaryFiles; query != nil {
		if err := aq.loadTemporaryFiles(ctx, query, nodes,
			func(n *Account) { n.Edges.TemporaryFiles = []*TemporaryFile{} },
			func(n *Account, e *TemporaryFile) { n.Edges.TemporaryFiles = append(n.Edges.TemporaryFiles, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withTenantAssignment; query != nil {
		if err := aq.loadTenantAssignment(ctx, query, nodes,
			func(n *Account) { n.Edges.TenantAssignment = []*TenantAccountAssignment{} },
			func(n *Account, e *TenantAccountAssignment) {
				n.Edges.TenantAssignment = append(n.Edges.TenantAssignment, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (aq *AccountQuery) loadTenants(ctx context.Context, query *TenantQuery, nodes []*Account, init func(*Account), assign func(*Account, *Tenant)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(account.TenantsTable)
		s.Join(joinT).On(s.C(tenant.FieldID), joinT.C(account.TenantsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(account.TenantsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(account.TenantsPrimaryKey[0]))
...
func (aq *AccountQuery) loadReceivedMails(ctx context.Context, query *MailQuery, nodes []*Account, init func(*Account), assign func(*Account, *Mail)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mail.FieldReceiverID)
	}
	query.Where(predicate.Mail(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.ReceivedMailsColumn), fks...))
	}))
...
func (aq *AccountQuery) loadTemporaryFiles(ctx context.Context, query *TemporaryFileQuery, nodes []*Account, init func(*Account), assign func(*Account, *TemporaryFile)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(temporaryfile.FieldOwnerID)
	}
	query.Where(predicate.TemporaryFile(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.TemporaryFilesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (aq *AccountQuery) loadTenantAssignment(ctx context.Context, query *TenantAccountAssignmentQuery, nodes []*Account, init func(*Account), assign func(*Account, *TenantAccountAssignment)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(tenantaccountassignment.FieldAccountID)
	}
	query.Where(predicate.TenantAccountAssignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.TenantAssignmentColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
