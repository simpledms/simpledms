...
func (aq *AttributeQuery) QuerySpace() *SpaceQuery {
	query := (&SpaceClient{config: aq.config}).Query()
...
func (aq *AttributeQuery) QueryDocumentType() *DocumentTypeQuery {
	query := (&DocumentTypeClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AttributeQuery) QueryTag() *TagQuery {
...
func (aq *AttributeQuery) QueryProperty() *PropertyQuery {
...
func (aq *AttributeQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (aq *AttributeQuery) Clone() *AttributeQuery {
	...
	return &AttributeQuery{
		...
		withSpace:        aq.withSpace.Clone(),
		withDocumentType: aq.withDocumentType.Clone(),
		withTag:          aq.withTag.Clone(),
		withProperty:     aq.withProperty.Clone(),
...
func (aq *AttributeQuery) WithSpace(opts ...func(*SpaceQuery)) *AttributeQuery {
	query := (&SpaceClient{config: aq.config}).Query()
...
func (aq *AttributeQuery) WithDocumentType(opts ...func(*DocumentTypeQuery)) *AttributeQuery {
	query := (&DocumentTypeClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withDocumentType = query
	return aq
}
...
func (aq *AttributeQuery) WithTag(opts ...func(*TagQuery)) *AttributeQuery {
	query := (&TagClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AttributeQuery) WithProperty(opts ...func(*PropertyQuery)) *AttributeQuery {
	query := (&PropertyClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withProperty = query
	return aq
}
...
//		GroupBy(attribute.FieldSpaceID).
...
func (aq *AttributeQuery) prepareQuery(ctx context.Context) error {
	...
	if attribute.Policy == nil {
		return errors.New("enttenant: uninitialized attribute.Policy (forgotten import enttenant/runtime?)")
	}
	if err := attribute.Policy.EvalQuery(ctx, aq); err != nil {
		return err
	}
	return nil
}
...
func (aq *AttributeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attribute, error) {
	...
	if query := aq.withSpace; query != nil {
		if err := aq.loadSpace(ctx, query, nodes, nil,
			func(n *Attribute, e *Space) { n.Edges.Space = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withDocumentType; query != nil {
		if err := aq.loadDocumentType(ctx, query, nodes, nil,
			func(n *Attribute, e *DocumentType) { n.Edges.DocumentType = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withTag; query != nil {
		if err := aq.loadTag(ctx, query, nodes, nil,
			func(n *Attribute, e *Tag) { n.Edges.Tag = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withProperty; query != nil {
		if err := aq.loadProperty(ctx, query, nodes, nil,
			func(n *Attribute, e *Property) { n.Edges.Property = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttributeQuery) loadSpace(ctx context.Context, query *SpaceQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *Space)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Attribute)
	for i := range nodes {
		fk := nodes[i].SpaceID
...
func (aq *AttributeQuery) loadDocumentType(ctx context.Context, query *DocumentTypeQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *DocumentType)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Attribute)
	for i := range nodes {
...
func (aq *AttributeQuery) loadTag(ctx context.Context, query *TagQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *Tag)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Attribute)
	for i := range nodes {
		fk := nodes[i].TagID
...
func (aq *AttributeQuery) loadProperty(ctx context.Context, query *PropertyQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *Property)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Attribute)
	for i := range nodes {
...
func (aq *AttributeQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(attribute.Table, attribute.Columns, sqlgraph.NewFieldSpec(attribute.FieldID, field.TypeInt64))
	_spec.From = aq.sql
	...
	if fields := aq.ctx.Fields; len(fields) > 0 {
		...
		if aq.withSpace != nil {
			_spec.Node.AddColumnOnce(attribute.FieldSpaceID)
		}
		if aq.withDocumentType != nil {
			_spec.Node.AddColumnOnce(attribute.FieldDocumentTypeID)
		}
		if aq.withTag != nil {
			_spec.Node.AddColumnOnce(attribute.FieldTagID)
		}
		if aq.withProperty != nil {
			_spec.Node.AddColumnOnce(attribute.FieldPropertyID)
		}
	}
