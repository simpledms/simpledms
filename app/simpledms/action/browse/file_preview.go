...
func NewFilePreview(infra *common.Infra, actions *Actions) *FilePreview {
	return &FilePreview{
		infra,
		actions,
		actionx.NewConfig(
...
func (qq *FilePreview) Data(currentDirID, fileID string) *FilePreviewData {
...
func (qq *FilePreview) Handler(rw httpx.ResponseWriter, req *httpx.Request, ctx ctxx.Context) error {
	data, err := autil.FormData[FilePreviewData](rw, req, ctx)
	if err != nil {
		return err
	}
...
func (qq *FilePreview) appBar(ctx ctxx.Context, dirID string, title *wx.Text, filex *model.File) *wx.AppBar {
	return &wx.AppBar{
		...
		Actions: []wx.IWidget{
			&wx.IconButton{
				...
				HTMXAttrs: wx.HTMXAttrs{
					DialogID: qq.actions.FileDetailsSideSheet.ID(),
				},
			},
			&wx.Link{
