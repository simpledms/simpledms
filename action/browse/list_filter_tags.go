...
type ListFilterTagsData struct {
	CurrentDirID string
}

type ListFilterTagsState struct {
	// int instead of int64 because of query (sql.InInts)
	// TODO is this okay? should be if we run on 64 bit system
	CheckedTagIDs []int `url:"tag_ids,omitempty"` // shared with DocumentTypeFilterState
}

type ListFilterTags struct {
	infra   *common.Infra
	actions *Actions
	*actionx.Config
}

func NewListFilterTags(infra *common.Infra, actions *Actions) *ListFilterTags {
...
func (qq *ListFilterTags) Data(currentDirID string) *ListFilterTagsData {
	return &ListFilterTagsData{
		CurrentDirID: currentDirID,
	}
}

func (qq *ListFilterTags) Handler(rw httpx.ResponseWriter, req *httpx.Request, ctx ctxx.Context) error {
	data, err := autil.FormData[ListFilterTagsData](rw, req, ctx)
	if err != nil {
		return err
	}
	state := autil.StateX[ListFilterTagsState](rw, req)
	...
	for _, tagx := range tagsInScope {
		...
		chip := &wx.FilterChip{
			...
			HTMXAttrs: wx.HTMXAttrs{
				...
				HxVals: util.JSON(qq.actions.ToggleTagFilter.Data(currentDirID, tagx.ID)),
				HxSwap: "none",
			},
		}
