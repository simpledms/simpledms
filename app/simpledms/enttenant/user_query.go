...
type UserQuery struct {
	...
	withSpaceAssignment *SpaceUserAssignmentQuery
...
func (uq *UserQuery) QuerySpaces() *SpaceQuery {
	query := (&SpaceClient{config: uq.config}).Query()
...
// QuerySpaceAssignment chains the current query on the "space_assignment" edge.
func (uq *UserQuery) QuerySpaceAssignment() *SpaceUserAssignmentQuery {
	query := (&SpaceUserAssignmentClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withSpaces:          uq.withSpaces.Clone(),
		withSpaceAssignment: uq.withSpaceAssignment.Clone(),
...
func (uq *UserQuery) WithSpaces(opts ...func(*SpaceQuery)) *UserQuery {
	query := (&SpaceClient{config: uq.config}).Query()
...
// WithSpaceAssignment tells the query-builder to eager-load the nodes that are connected to
// the "space_assignment" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithSpaceAssignment(opts ...func(*SpaceUserAssignmentQuery)) *UserQuery {
	query := (&SpaceUserAssignmentClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withSpaceAssignment = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withSpaces; query != nil {
		if err := uq.loadSpaces(ctx, query, nodes,
			func(n *User) { n.Edges.Spaces = []*Space{} },
			func(n *User, e *Space) { n.Edges.Spaces = append(n.Edges.Spaces, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withSpaceAssignment; query != nil {
		if err := uq.loadSpaceAssignment(ctx, query, nodes,
			func(n *User) { n.Edges.SpaceAssignment = []*SpaceUserAssignment{} },
			func(n *User, e *SpaceUserAssignment) { n.Edges.SpaceAssignment = append(n.Edges.SpaceAssignment, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadSpaces(ctx context.Context, query *SpaceQuery, nodes []*User, init func(*User), assign func(*User, *Space)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.SpacesTable)
		s.Join(joinT).On(s.C(space.FieldID), joinT.C(user.SpacesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.SpacesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.SpacesPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Space](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "spaces" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadSpaceAssignment(ctx context.Context, query *SpaceUserAssignmentQuery, nodes []*User, init func(*User), assign func(*User, *SpaceUserAssignment)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(spaceuserassignment.FieldUserID)
	}
	query.Where(predicate.SpaceUserAssignment(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.SpaceAssignmentColumn), fks...))
	}))
