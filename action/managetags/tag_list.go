...
func NewTagList(infra *common.Infra, actions *Actions) *TagList {
...
func (qq *TagList) Handler(rw httpx.ResponseWriter, req *httpx.Request, ctx ctxx.Context) error {
	data, err := autil.FormData[TagListData](rw, req, ctx)
	if err != nil {
		return err
	}
	state := autil.StateX[TagListState](rw, req)

	return qq.infra.Renderer().Render(rw, ctx, qq.Widget(ctx, data, state))
}

func (qq *TagList) Data(parentTagID int64) *TagListData {
	return &TagListData{
		ParentTagID: parentTagID,
	}
}

func (qq *TagList) Widget(ctx ctxx.Context, data *TagListData, state *TagListState) *wx.List {
...
func (qq *TagList) listItem(
	...
	if tagx.Type == tagtype.Group {
		...
		childItems = append(childItems, &wx.ListItem{
			...
			HTMXAttrs: qq.actions.Tagging.CreateTag.ModalLinkAttrs(
				qq.actions.Tagging.CreateTag.Data(tagx.ID),
				"",
			),
		})
		...
		if isOpen {
			...
			htmxAttrs = wx.HTMXAttrs{
				HxPost:   qq.actions.ToggleTagGroup.Endpoint(),
				HxVals:   util.JSON(qq.actions.ToggleTagGroup.Data(tagx.ID)),
...
func (qq *TagList) listItemID(tagID int64) string {
	return fmt.Sprintf("tagListItem-%d", tagID)
}

func (qq *TagList) id() string {
	return "tagList"
}
