...
type EditAssignedTagsData struct {
	FileID      string
	ParentTagID int64 // to load group children
}

type EditAssignedTags struct {
	infra   *common.Infra
	actions *Actions
	*actionx.Config
}

func NewEditAssignedTags(infra *common.Infra, actions *Actions) *EditAssignedTags {
	config := actionx.NewConfig(
		actions.Route("edit-assigned-tags"),
		true, // TODO true or false?
	)
	return &EditAssignedTags{
		infra:   infra,
		actions: actions,
		Config:  config,
	}
...
func (qq *EditAssignedTags) Data(fileID string, parentTagID int64) *EditAssignedTagsData {
	return &EditAssignedTagsData{
		FileID:      fileID,
		ParentTagID: parentTagID,
	}
}

func (qq *EditAssignedTags) Handler(rw httpx.ResponseWriter, req *httpx.Request, ctx ctxx.Context) error {
	// TODO do prep if necessary... (filter selects, etc.)
	// TODO set default value, for example for current

	data, err := autil.FormData[EditAssignedTagsData](rw, req, ctx)
	if err != nil {
		return err
	}
...
func (qq *EditAssignedTags) Form(
	ctx ctxx.Context,
	data *EditAssignedTagsData,
...
func (qq *EditAssignedTags) bottomAppBar(
	data *EditAssignedTagsData,
) *wx.BottomAppBar {
...
func (qq *EditAssignedTags) ListView(
	ctx ctxx.Context,
	data *EditAssignedTagsData,
		...
		allListItems = append(allListItems,
			...
			&wx.ListItem{
				HTMXAttrs: qq.actions.AssignedTags.CreateAndAssignTag.ModalLinkAttrs(
					qq.actions.AssignedTags.CreateAndAssignTag.Data(data.FileID, 0),
					"#"+qq.hxTargetID(),
				),
	...
	if len(allListItems) == 1 { // check if only create list item is added
		return &wx.ScrollableContent{
			...
			Children: &wx.EmptyState{
				...
				Actions: []wx.IWidget{
					qq.actions.AssignedTags.CreateAndAssignTag.ModalLink(
						qq.actions.AssignedTags.CreateAndAssignTag.Data(data.FileID, 0),
						[]wx.IWidget{
							&wx.Button{
...
func (qq *EditAssignedTags) hxTargetID() string {
