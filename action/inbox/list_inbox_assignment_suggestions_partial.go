...
type ListInboxAssignmentSuggestionsPartial struct {
	infra   *common.Infra
	actions *Actions
	// *actionx.Config
}

func NewListInboxAssignmentSuggestionsPartial(infra *common.Infra, actions *Actions) *ListInboxAssignmentSuggestionsPartial {
	return &ListInboxAssignmentSuggestionsPartial{
		infra:   infra,
		actions: actions,
...
func (qq *ListInboxAssignmentSuggestionsPartial) Data(fileID int64) *ListInboxAssignmentSuggestionsPartialData {
	return &ListInboxAssignmentSuggestionsPartialData{
		FileID: fileID,
	}
}

/*
func (qq *ListInboxAssignmentSuggestionsPartial) Handler(rw httpx.ResponseWriter, req *httpx.Request, ctx *ctxx.Context) error {
	data, err := autil.FormData[ListInboxAssignmentSuggestionsPartialData](rw, req, ctx)
	if err != nil {
		return err
	}
...
func (qq *ListInboxAssignmentSuggestionsPartial) Widget(ctx ctxx.Context, fileID int64) *wx.List {
	...
	*/
	...
	for _, destDir := range destDirs {
		...
		items = append(items, qq.actions.AssignmentDirectoryListItemPartial.Widget(
			ctx,
			destDir,
			fileToAssign,
