...
func NewTagContextMenu(actions *Actions) *TagContextMenu {
...
func (qq *TagContextMenu) Widget(ctx ctxx.Context, tagx *enttenant.Tag) *wx.Menu {
	deleteLink := &wx.MenuItem{
		LeadingIcon: "delete",
		Label:       wx.T("Delete"),
		HTMXAttrs: wx.HTMXAttrs{
			HxPost:    qq.actions.Tagging.DeleteTag.Endpoint(),
			HxVals:    util.JSON(qq.actions.Tagging.DeleteTag.Data(tagx.ID)),
	...
	menuItems := []*wx.MenuItem{
		{
			LeadingIcon: "edit",
			Label:       wx.T("Edit"),
			HTMXAttrs: qq.actions.Tagging.EditTag.ModalLinkAttrs(
				qq.actions.Tagging.EditTag.Data(tagx.ID, tagx.Name),
				"",
			),
		},
	}
		...
		if groupCount > 0 {
			menuItems = append(
				...
				&wx.MenuItem{
					...
					HTMXAttrs: qq.actions.Tagging.MoveTagToGroup.ModalLinkAttrs(
						qq.actions.Tagging.MoveTagToGroup.Data(tagx.ID, 0),
						"",
					),
				},
			)
