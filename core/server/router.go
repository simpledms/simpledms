...
type (
	handlerFn  func(rw httpx2.ResponseWriter, req *httpx2.Request, ctx ctxx.Context) error
...
func (qq *Router) wrapCommand(handlerFn handlerFn) handlerFn {
	return func(rw httpx2.ResponseWriter, req *httpx2.Request, ctx ctxx.Context) error {
...
func (qq *Router) wrapTx(handlerFn handlerFn, isReadOnly bool) http.HandlerFunc {
	return func(rw http.ResponseWriter, req *http.Request) {
		...
		*/
		...
		rwx := httpx2.NewResponseWriter(rw)
		reqx := httpx2.NewRequest(req)
...
func (qq *Router) context(
	...
	if isTemporarySession {
		...
		if isTenantPasskeyEnrollmentRequired && !qq.isSetupSessionPathAllowed(req.URL.Path) {
			...
			http.Redirect(rw, req.Request, route.Dashboard(), http.StatusSeeOther)
			return mainCtx, nil, true, nil
		}
	}
	...
	tenantCtx := ctxx2.NewTenantContext(mainCtx, tenantTx, tenantx, isReadOnly)
	if spaceID == "" {
